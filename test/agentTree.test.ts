...
import { AgentTree, ROOT_ID } from "../src/data/agentTree.ts";
import { parseLine } from "../src/data/transcript.ts";
...
describe("AgentTree", () => {
  ...
  test("flags files written by more than one agent", () => {
    ...
    t.addRecord(write("toolu_A", "/repo/shared.ts"));
    t.addRecord(write("toolu_B", "/repo/shared.ts"));
    t.addRecord(write("toolu_A", "/repo/only-a.ts"));
    const col = t.fileCollisions();
    expect([...col.keys()]).toEqual(["/repo/shared.ts"]);
    expect(col.get("/repo/shared.ts")).toHaveLength(2);
  });
});
